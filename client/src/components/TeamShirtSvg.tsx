import React from 'react';

interface TeamShirtSvgProps {
  size?: number;
  primaryColor?: string;
  secondaryColor?: string;
  teamId?: number;
  team?: {
    id?: number;
    primaryColor?: string;
    secondaryColor?: string;
  };
}

const TeamShirtSvg: React.FC<TeamShirtSvgProps> = ({
  size = 56,
  primaryColor,
  secondaryColor,
  teamId,
  team,
}) => {
  // Algoritmo para generar color desde id
  function idToColor(id: number, offset = 0): string {
    const hue = ((id * 137 + offset) % 360);
    return `hsl(${hue}, 70%, 48%)`;
  }

  // Extraer id si existe
  const resolvedId = teamId || team?.id;

  // Extrae colores de posibles campos alternativos
  // Permite acceder a campos alternativos aunque TS no los declare
  const teamAny = team as any;
  let teamPrimary = team?.primaryColor || teamAny?.primary_color;
  let teamSecondary = team?.secondaryColor || teamAny?.secondary_color;
  // Si no existen, intenta extraer de clubColors (formato: "#ff0000 / #0000ff" o similar)
  const clubColors = teamAny?.clubColors || teamAny?.club_colors;
  if ((!teamPrimary || !teamSecondary) && clubColors) {
    const parts = clubColors.split(/[\/,|]/).map((s: string) => s.trim());
    if (typeof window !== 'undefined') {
      // eslint-disable-next-line no-console
      console.log('[TeamShirtSvg] clubColors', { clubColors, parts });
    }
    if (!teamPrimary && parts[0]) teamPrimary = parts[0];
    if (!teamSecondary && parts[1]) teamSecondary = parts[1];
  }

  // Prioridad: team, luego props, luego calculados (idéntico a TeamCrestSvg)
  let color1 = teamPrimary || primaryColor;
  let color2 = teamSecondary || secondaryColor;
  if (!color1 && resolvedId) color1 = idToColor(resolvedId, 0);
  if (!color2 && resolvedId) color2 = idToColor(resolvedId, 180);
  if (!color1) color1 = '#1e90ff';
  if (!color2) color2 = '#222222';

  // Debug: log color resolution
  if (typeof window !== 'undefined') {
    // eslint-disable-next-line no-console
    console.log('[TeamShirtSvg]', {
      team,
      teamKeys: team ? Object.keys(team) : undefined,
      teamPrimaryColor: team?.primaryColor,
      teamSecondaryColor: team?.secondaryColor,
      teamRaw: JSON.stringify(team),
      primaryColor,
      secondaryColor,
      resolvedId,
      color1,
      color2
    });
  }

  return (
    <svg width={size} height={size} viewBox="0 -7.72 127.24603 127.24603" xmlns="http://www.w3.org/2000/svg">
      <g>
        {/* Parte principal de la camiseta (color1) */}
        <path fill={color1} d="m32 109c-1.4 0-2.5-1.1-2.5-2.5v-62.6l-7 5.9c-.1.1-.2.1-.2.2-1 .6-2.1 1-3.2 1-2 0-3.8-1-4.9-2.6l-10.7-16.1c-1.8-2.7-1-6.3 1.7-8.1l29.7-20.7c.2-.1.4-.3.7-.3.1 0 2.8-.9 6.6-1h3.1c.7 0 1.4.3 1.8.8.5.5.7 1.2.6 1.9 0 .1 0 .3-.1.4.2 7.5 8.1 14.5 16.5 14.5s16.3-7 16.5-14.5c0-.1 0-.3-.1-.4-.1-.7.2-1.4.6-1.9s1.1-.8 1.8-.8h6.1c2.4 0 4.3 1.1 4.5 1.2.1 0 .1.1.2.1l28.7 20.7c2.6 1.7 3.4 5.4 1.6 8.1l-10.7 15.2c-1 1.6-2.9 2.6-4.9 2.6-1.2 0-2.3-.3-3.2-1-.1 0-.1-.1-0.2-.2l-6.4-5.3-.2 62.9c0 1.4-1.1 2.5-2.5 2.5h-63.9z"/>
        {/* Parte secundaria (negro) */}
        <path fill="#000" d="m44.7 2.5v106l13 1v-90.2c-5.7-2.4-10-7.7-10.1-13.2.1-.10.1-.30.1-.40.1-.70-.1-1.4-.6-1.9-.4-.50-1.1-.80-1.8-.80h-.6zm38.2 0c-.7 0-1.4.30-1.8.80s-.7 1.2-.6 1.9c.1.10.1.30.1.40-.2 5.5-4.5 10.8-10.1 13.2v90.2h12.6v-106l-.2-.5z"/>
        {/* Detalles en color2 */}
        <path fill={color2} d="m44.7 2.5v106l13 1v-90.2c-5.7-2.4-10-7.7-10.1-13.2.1-.10.1-.30.1-.40.1-.70-.1-1.4-.6-1.9-.4-.50-1.1-.80-1.8-.80h-.6zm38.2 0c-.7 0-1.4.30-1.8.80s-.7 1.2-.6 1.9c.1.10.1.30.1.40-.2 5.5-4.5 10.8-10.1 13.2v90.2h12.6v-106l-.2-.5zm13 2.94v2.86l28.1 19.7-7 10.2-21.1-14.8v85.6c1.4 0 2.5-1 2.5-2l.2-63.1 6.4 5.3v.2c1 .7 2 1 3 1 2 0 4-1 5-2.6l11-15.2c2-2.7 1-6.4-2-8.1l-26.1-19.1zm-64 .46l-26.7 18.6c-2.73 1.8-3.53 5.4-1.73 8.1l10.7 16.1c1.1 1.6 2.9 2.6 4.9 2.6 1.1 0 2.2-.4 3.2-1 0-.1.1-.1.2-.2l7-5.9v62.8c0 1 1 2 2.4 2v-85.2l-21.6 15.1-7.15-10.1 28.8-20.2-.1-2.7z"/>
        <path d="m89.1 5c1.8 0 3.1.9 3.1.9l28.7 20.6c1.6 1 2 3.1.9 4.7l-10.7 15.1c-0.6 1-1.7 1.5-2.8 1.5-.6 0-1.3-.2-1.9-.6l-10.5-8.6-.2 68.2h-63.7v-68l-11.2 9.4c-.6.4-1.2.6-1.9.6-1.1 0-2.2-.5-2.8-1.5l-10.6-16.1c-1-1.6-.6-3.6.9-4.7l29.7-20.7s2.4-.8 5.8-.9h3.1v.2.2c0 9 9.1 17.3 19 17.3s19-8.3 19-17.3v-.2-.2h5.9.2c-.1.1 0 .1 0 .1m0-5s-.1 0 0 0h-.2-5.9c-1.4 0-2.7.6-3.7 1.6-.9 1-1.4 2.4-1.3 3.8v.4c-.3 6.1-7.1 11.9-14 11.9s-13.7-5.8-14-11.9v-.4c.1-1.4-.3-2.8-1.3-3.8-.9-1-2.3-1.6-3.7-1.6h-3.1-.1c-4 .1-6.9 1-7.3 1.1-.5.2-.9.4-1.3.7l-29.5 20.6c-3.8 2.6-4.8 7.7-2.3 11.6l10.7 16.1c1.6 2.3 4.2 3.7 7 3.7 1.6 0 3.2-.5 4.6-1.4.2-.1.3-.2.5-.3l2.9-2.5v57.2c0 2.8 2.2 5 5 5h63.8c2.8 0 5-2.2 5-5l.1-57.7 2.3 1.9c.1.1.3.2.4.3 1.4.9 3 1.4 4.6 1.4 2.8 0 5.4-1.4 6.9-3.7l10.6-15 .1-.1c2.5-3.8 1.5-9-2.3-11.5l-28-20.9c-.1-.1-.2-.1-.3-.2-.3-.2-2.6-1.5-5.6-1.5z"/>
      </g>
    </svg>
  );
};

export default TeamShirtSvg;
